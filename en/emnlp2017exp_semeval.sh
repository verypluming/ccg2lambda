...
for dataset in {trial,train,test}; do
  # Run pipeline for each entailment problem.
  for ff in ${plain_dir}/sick_${dataset}.files_??; do
    for f in `cat ${ff}`; do
      ./en/similarity_en_mp_any_semeval.sh $f $templates $word2vec;
    done &
  done
...
python scripts/randomforest_all_semeval.py --results $results_dir
