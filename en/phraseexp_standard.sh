...
for ff in ${plain_dir}/sick_${dataset}.files_??; do
  for f in `cat ${ff}`; do
    ./en/rte_en_mp_any_phrase_standard.sh $f $templates ${dataset} ${plain_dir} ${parsed_dir} ${results_dir};
  done &
done
...
./ja/accuracy.sh ${results_dir}/main_${dataset}.html > ${results_dir}/score_${dataset}.txt
