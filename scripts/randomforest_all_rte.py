...
def retrieve_features(recalc=None, sick_train=None, sick_test=None):
    if recalc:
        ...
        with open('results/dep_1de7261/features_np.pickle', 'rb') as in_f:
...
def output_errors(outputs, sick_ids, trial_targets):
    """
...
def main():
    ...
    train_sources, train_targets, trial_sources, trial_targets, train_id, trial_id = retrieve_features()
    ...
    f = open("./results/rte_report.txt", "w")
    f.write(classification_report(trial_targets, outputs, digits=5))
    f.write(str(accuracy_score(trial_targets, outputs)))
    f.close()
    
    # Evaluate regressor
    write_for_evaluation(outputs, trial_id, trial_targets) #Outputs and sick_ids

    # Check errors
    output_errors(outputs, trial_id, trial_targets) #Outputs and sick_ids
