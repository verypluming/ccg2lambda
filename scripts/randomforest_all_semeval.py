...
def load_sick_data_from(sick_id, kind, results):
    ...
    print(kind, sick_id)
    line.append(sick_id)
    f = open('./plain/sick_'+kind.lower()+'_'+sick_id+'.answer', 'r')
...
def load_sick_data(results):
    ...
    filelist = glob.glob('./'+results+'/sick*.answer')
    for filename in filelist:
        ...
        if re.search("candc", filename):
            ...
            continue
        if load_sick_data_from(filename, "TRIAL", results) is not None:
            sick_train.append(load_sick_data_from(filename, "TRIAL", results))
        elif load_sick_data_from(filename, "TRAIN", results) is not None:
            sick_train.append(load_sick_data_from(filename, "TRAIN", results))
        elif load_sick_data_from(filename, "TEST", results) is not None:
            sick_test.append(load_sick_data_from(filename, "TEST", results))
