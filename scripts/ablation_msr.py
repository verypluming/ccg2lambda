...
def retrieve_features(train, trial, recalc=None, sick_train=None, sick_test=None):
    if recalc:
        ...
        with open('vid_results/features_np.pickle', 'rb') as in_f:
...
def write_for_evaluation(outputs, sick_ids, trial_targets, name):
    with open('./vid_results/'+name+'_all_result.txt', 'w') as out_f:
        ...
        for i, line in enumerate(outputs):
            ...
            if os.path.isfile('./vid_plain/sick_test_'+sick_ids[i]+'.answer'):
                ...
                j.close()
                out_f.write('{0}\t{1}\t{2}\t{3}\n'.format(sick_ids[i], entailment, data, trial_targets[i]))
            elif os.path.isfile('./vid_plain/sick_trial_'+sick_ids[i]+'.answer'):
                j = open('./vid_plain/sick_trial_'+sick_ids[i]+'.answer', 'r')
                entailment = j.readlines()[0].strip()
                j.close()
                out_f.write('{0}\t{1}\t{2}\t{3}\n'.format(sick_ids[i], entailment, data, trial_targets[i]))
...
def output_errors(outputs, gold, sick_ids, sick_sentences, name):
    with open('./vid_results/'+name+'_error_result.txt', 'w') as out_f:
...
def main():
    ...
    for command in commands:
        ...
        with open('./vid_results/'+name+'_evaluation.txt', 'w') as eval_f:
            ## pearson correlation
            r, p = pearsonr(x, y)
